...
namespace T1.B1.WithholdingTax
{
    ...
    {
        static private WithholdingTax objWithHoldingTax = null;

        static private Hashtable CardCodeHash = null;

        private WithholdingTax()
        {
            CardCodeHash = new Hashtable();
        }
        ...
        static public void ItemEvent(string FormUID, ref SAPbouiCOM.ItemEvent pVal, out bool BubbleEvent)
        {
            ...
            try
            {
                ...
                #region Add Missing SelfWithholding Tax
                if (pVal.FormTypeEx == "BYB_WHTFRM004")
                {
                    ...
                    else
                    {
                        if (pVal.EventType == SAPbouiCOM.BoEventTypes.et_ITEM_PRESSED)
                        {
                            ...
                            else if(pVal.ItemUID == "btnGet")
                            {
                                getSelfWithholdingTaxDocuments(BYBB1MainObject.Instance.B1Application.Forms.Item(pVal.FormUID));
                            }
                        }
                ...
                T1.B1.WithholdingTax.Report.ReportMainClass.ItemEvent(ref pVal, out BubbleEvent);
            }
        ...
        static private void addTaxandWTAdjust(SAPbouiCOM.ItemEvent pVal)
        {
            ...
            SAPbouiCOM.UserDataSource oDocContabDate = null;
            ...
            try
            {
                ...
                oDocContabDate = objForm.DataSources.UserDataSources.Item("UD_DT");
                ...
                if (oNodes != null && oNodes.Count > 0)
                {
                    foreach (XmlNode xn in oNodes)
                    {
                            ...
                            )
                        {
                            ...
                            oJournal.TaxDate = new DateTime(Convert.ToInt32(oDocContabDate.ValueEx.Substring(0,4)), Convert.ToInt32(oDocContabDate.ValueEx.Substring(4, 2)), Convert.ToInt32(oDocContabDate.ValueEx.Substring(6, 2)));// Convert.ToDateTime(    );
                            oJournal.ReferenceDate = new DateTime(Convert.ToInt32(oDocContabDate.ValueEx.Substring(0, 4)), Convert.ToInt32(oDocContabDate.ValueEx.Substring(4, 2)), Convert.ToInt32(oDocContabDate.ValueEx.Substring(6, 2)));// Convert.ToDateTime(    );
                            oJournal.DueDate = new DateTime(Convert.ToInt32(oDocContabDate.ValueEx.Substring(0, 4)), Convert.ToInt32(oDocContabDate.ValueEx.Substring(4, 2)), Convert.ToInt32(oDocContabDate.ValueEx.Substring(6, 2)));// Convert.ToDateTime(    );
        ...
        static public void addMenu()
        {
            ...
            try
            {
                #region Withholding Tax Main Menu
                strMenuId = B1.WithholdingTax.InteractionId.Default.mnuWHTMainMenuId;
                if (!BYBB1MainObject.Instance.B1Application.Menus.Exists(strMenuId))
                {
                    strMenuDescription = B1.WithholdingTax.MessageStrings.Default.mnuWHTMainMenuDescription;
                    ...
                    int intTotal = BYBB1MainObject.Instance.B1Application.Menus.Item(B1.Shared.InteractionId.Default.T1MainFolderMenuId).SubMenus.Count + 1;
                    objMenuCreationParams.Position = intTotal;


                    BYBB1MainObject.Instance.B1Application.Menus.Item(B1.Shared.InteractionId.Default.T1MainFolderMenuId).SubMenus.AddEx(objMenuCreationParams);
                }
                #endregion Withholding Tax Main Menu

                #region Withholding Tax Report Main Menu
                strMenuId = B1.WithholdingTax.InteractionId.Default.mnuWHTReportId;
                if (!BYBB1MainObject.Instance.B1Application.Menus.Exists(strMenuId))
                {
                    strMenuDescription = B1.WithholdingTax.MessageStrings.Default.mnuWHTReportDescription;

                    SAPbouiCOM.MenuCreationParams objMenuCreationParams = BYBB1MainObject.Instance.B1Application.CreateObject(SAPbouiCOM.BoCreatableObjectType.cot_MenuCreationParams);
                    objMenuCreationParams.String = strMenuDescription;
                    objMenuCreationParams.UniqueID = strMenuId;
                    objMenuCreationParams.Type = SAPbouiCOM.BoMenuType.mt_POPUP;
                    int intTotal = BYBB1MainObject.Instance.B1Application.Menus.Item(B1.WithholdingTax.InteractionId.Default.mnuWHTMainMenuId).SubMenus.Count + 1;
                    objMenuCreationParams.Position = intTotal;


                    BYBB1MainObject.Instance.B1Application.Menus.Item(B1.WithholdingTax.InteractionId.Default.mnuWHTMainMenuId).SubMenus.AddEx(objMenuCreationParams);
                }
                #endregion Withholding Tax Report Main Menu


                strMenuId = "BYBMNU011";
                if (!BYBB1MainObject.Instance.B1Application.Menus.Exists(strMenuId))
                {
                    strMenuDescription = "Impuestos";

                    SAPbouiCOM.MenuCreationParams objMenuCreationParams = BYBB1MainObject.Instance.B1Application.CreateObject(SAPbouiCOM.BoCreatableObjectType.cot_MenuCreationParams);
                    objMenuCreationParams.String = strMenuDescription;
                    objMenuCreationParams.UniqueID = strMenuId;
                    objMenuCreationParams.Type = SAPbouiCOM.BoMenuType.mt_POPUP;
                    int intTotal = BYBB1MainObject.Instance.B1Application.Menus.Item("8704").SubMenus.Count + 1;
                    objMenuCreationParams.Position = intTotal;
                ...
                if (!BYBB1MainObject.Instance.B1Application.Menus.Exists(strMenuId))
                {
                    strMenuDescription = "Calcular Autoretenciones faltantes";
                    ...
                    int intTotal = BYBB1MainObject.Instance.B1Application.Menus.Item("BYBMNU011").SubMenus.Count + 1;
                    objMenuCreationParams.Position = intTotal;


                    BYBB1MainObject.Instance.B1Application.Menus.Item("BYBMNU011").SubMenus.AddEx(objMenuCreationParams);
                }

                strMenuId = "BYBMNU013";
                if (!BYBB1MainObject.Instance.B1Application.Menus.Exists(strMenuId))
                {
                    strMenuDescription = "Validar inconsistencias de retenciones";
                ...
                if (!BYBB1MainObject.Instance.B1Application.Menus.Exists(strMenuId))
                {
                    strMenuDescription = "Autoretenciones";
                    ...
                    objMenuCreationParams.UniqueID = strMenuId;
                    objMenuCreationParams.Type = SAPbouiCOM.BoMenuType.mt_STRING;
                    int intTotal = BYBB1MainObject.Instance.B1Application.Menus.Item("15616").SubMenus.Count + 1;
                    objMenuCreationParams.Position = intTotal;


                    BYBB1MainObject.Instance.B1Application.Menus.Item("15616").SubMenus.AddEx(objMenuCreationParams);
                }

                strMenuId = B1.WithholdingTax.InteractionId.Default.mnuWHTMasterFormId;
                if (!BYBB1MainObject.Instance.B1Application.Menus.Exists(strMenuId))
                {
                    strMenuDescription = B1.WithholdingTax.LocalizationStrings.Default.mnuWHTMasterFormString;
                    strMenuId = InteractionId.Default.mnuWHTMasterFormId;
                    ...
                    BYBB1MainObject.Instance.B1Application.Menus.Item(InteractionId.Default.mnuWHTMasterParent).SubMenus.AddEx(objMenuCreationParams);
                }

                //B1.WithholdingTax.Report.ReportMainClass.addMenu();
        ...
        static public void MenuEvent(ref SAPbouiCOM.MenuEvent pVal, out bool BubbleEvent)
        {
            ...
            try
            {
                ...
                if (!pVal.BeforeAction)
                {
                    if (pVal.MenuUID == InteractionId.Default.mnuWHTMasterFormId)
                    {
                        ...
                        objFormCreationParams.XmlData = localForm(InteractionId.Default.frmWHTMasterFormId);
                        objFormCreationParams.FormType = InteractionId.Default.frmWHTMasterFormType;
                    ...
                    else if (pVal.MenuUID == "bybwtm002")
                    {
                        objForm = BYBB1MainObject.Instance.B1Application.Forms.ActiveForm;
                        SAPbouiCOM.DBDataSource objDS = objForm.DataSources.DBDataSources.Item("@BYB_T1WHT101");
                    ...
                    else if (pVal.MenuUID == "bybwtm004")
                    {
                        objForm = BYBB1MainObject.Instance.B1Application.Forms.ActiveForm;
                        SAPbouiCOM.DataTable objDT = objForm.DataSources.DataTables.Item("dtNew");
                        objDT.Rows.Add();


                    }
                    else if (pVal.MenuUID == "BYBMNU010")
                    {
                        SAPbouiCOM.FormCreationParams objFormCreationParams = null;
                        objFormCreationParams = BYBB1MainObject.Instance.B1Application.CreateObject(SAPbouiCOM.BoCreatableObjectType.cot_FormCreationParams);
                        objFormCreationParams.XmlData = localForm("BYB_WHTFRM003");
                        objFormCreationParams.FormType = "BYB_WHTFRM003";
                        objFormCreationParams.UniqueID = Guid.NewGuid().ToString().Substring(1, 20);
                        objForm = BYBB1MainObject.Instance.B1Application.Forms.AddEx(objFormCreationParams);

                        filterAccountChooseFromList(objForm);
                        addFirstLineToMatrixSWT(objForm);

                        objForm.Visible = true;

                    }

                    else if (pVal.MenuUID == "BYBMNU012")
                    {
                        SAPbouiCOM.FormCreationParams objFormCreationParams = null;
                        objFormCreationParams = BYBB1MainObject.Instance.B1Application.CreateObject(SAPbouiCOM.BoCreatableObjectType.cot_FormCreationParams);
                        objFormCreationParams.XmlData = localForm("BYB_WHTFRM004");
                        objFormCreationParams.FormType = "BYB_WHTFRM004";
                        objFormCreationParams.UniqueID = Guid.NewGuid().ToString().Substring(1, 20);
                        objForm = BYBB1MainObject.Instance.B1Application.Forms.AddEx(objFormCreationParams);
                        //getSelfWithholdingTaxDocuments(objForm);
                        objForm.Visible = true;


                    }
                    else if (pVal.MenuUID == "BYBMNU013")
                    {
                        SAPbouiCOM.FormCreationParams objFormCreationParams = null;
                        objFormCreationParams = BYBB1MainObject.Instance.B1Application.CreateObject(SAPbouiCOM.BoCreatableObjectType.cot_FormCreationParams);
                        objFormCreationParams.XmlData = localForm("BYB_WHTFRM006");
                        objFormCreationParams.FormType = "BYB_WHTFRM006";
                        objFormCreationParams.UniqueID = Guid.NewGuid().ToString().Substring(1, 20);
                        objForm = BYBB1MainObject.Instance.B1Application.Forms.AddEx(objFormCreationParams);
                        objForm.Visible = true;
                    ...
                    else if (pVal.MenuUID == "bybwtm003")
                    {
                        SAPbouiCOM.FormCreationParams objFormCreationParams = null;
                        objFormCreationParams = BYBB1MainObject.Instance.B1Application.CreateObject(SAPbouiCOM.BoCreatableObjectType.cot_FormCreationParams);
                        objFormCreationParams.XmlData = localForm("BYB_WHTFRM005");
                        objFormCreationParams.FormType = "BYB_WHTFRM005";
                        objFormCreationParams.UniqueID = Guid.NewGuid().ToString().Substring(1, 20);
                        objForm = BYBB1MainObject.Instance.B1Application.Forms.AddEx(objFormCreationParams);
                        getWithholdingInfo(objForm);
                        objForm.Visible = true;
                    }
                    #region SelfWithholdingTax Definition Window
                    else if (pVal.MenuUID == "BYBSWTM80")
                    {
                        addNewLineSWTBP();


                    }
                    else if (pVal.MenuUID == "BYBSWTM81")
                    {
                    ...
                    B1.WithholdingTax.Report.ReportMainClass.MenuEvent(ref pVal, out BubbleEvent);
                }
        ...
        static private string localForm(string strFormId)
        {
            ...
            try
            {
                ...
                if (strFormId == B1.WithholdingTax.InteractionId.Default.frmWHTMasterFormId)
                {
                    strResult = B1.WithholdingTax.Resources.WithholdingTax.WHTFRM001;

                }
                else if (strFormId == "BYB_WHTFRM003")
                {
                    strResult = B1.WithholdingTax.Resources.WithholdingTax.WHTFRM003;
                }
                else if (strFormId == "BYB_WHTFRM004")
                {
                    strResult = B1.WithholdingTax.Resources.WithholdingTax.WHTFRM004;
                }
                else if (strFormId == "BYB_WHTFRM005")
                {
                    strResult = B1.WithholdingTax.Resources.WithholdingTax.WHTFRM005;
                }
                else if (strFormId == "BYB_WHTFRM006")
                {
                    strResult = B1.WithholdingTax.Resources.WithholdingTax.WHTFRM006;
                }
        ...
        static private void getWithholdingInfo(SAPbouiCOM.Form objForm)
        {
            ...
            try
            {
                ...
                else if (objInvoiceForm.BusinessObject.Type == "13")
                {
                    #region Sales Invoice Adjust
                    ...
                    #endregion  Sales Invoice Adjust
                }
        ...
        static private void getSelfWithholdingTaxDocuments(SAPbouiCOM.Form objForm)
        {
            ...
            SAPbouiCOM.UserDataSource objFromDate = null;
            SAPbouiCOM.UserDataSource objToDate = null;
            ...
            try
            {
                objFromDate = objForm.DataSources.UserDataSources.Item("getInDate");
                strFromdate = objFromDate.ValueEx.Trim();
                objToDate = objForm.DataSources.UserDataSources.Item("getEndDate");
                strToDate = objToDate.ValueEx.Trim();
                objDS = objForm.DataSources.DBDataSources.Item("@BYB_T1SWT100");
                strSWTCode = objDS.GetValue("Code", objDS.Offset).Trim();

                string strQuery = B1.WithholdingTax.Resources.dbQueries.sqlGetMissingSWT;
                strQuery = strQuery.Replace("[--SWTCode--]", strSWTCode);
                ...
                if (objDT.Rows.Count > 0)
                {
                    ...
                    objGridColumn = objGrid.Columns.Item(1);
                    objGridColumn.Editable = false;
                    objGridColumn.RightJustified = false;
                    ...
                    objForm.Items.Item("Item_0").Visible = true;
                ...
                {
                    BYBB1MainObject.Instance.B1Application.MessageBox("No se encontraron documentos faltantes en la fecha especificada");
                }
        ...
        static private void addMissingSelfWithHolding(SAPbouiCOM.ItemEvent pVal)
        {
            ...
            try
            {
                    ...
                    if (strDateValue.Trim().Length > 0 || strDocDate == "Y")
                    {
                        ...
                        foreach (XmlNode xn in objNodes)
                        {
                            ...
                            if (objDocuments.RecordCount > 0)
                            {
                                ...
                                strSql = strSql.Replace("[--SWTCode--]", strSWTCode);

                                objRecordset.DoQuery(strSql);

                                if (objRecordset.RecordCount > 0)
                                {
                                    #region recalculte SelfWithholding
                                    objRecordset.MoveFirst();
                                    while (!objRecordset.EoF)
                                    {
                                        intObjectType = Convert.ToInt32(oDoc.DocObjectCodeEx);
                                        ...
                                        #region find if there is a strCode transaction not associated

                                        string strSQL = "select LocTotal,transid from OJDT where Memo = 'strCode' and refDate = '" + oDoc.DocDate.ToString("yyyyMMdd") + "'";
                                        SAPbobsCOM.Recordset objRecordSetNotLinked = BYBB1MainObject.Instance.B1Company.GetBusinessObject(SAPbobsCOM.BoObjectTypes.BoRecordset);
                                        objRecordSetNotLinked.DoQuery(strSQL);
                                        if(objRecordSetNotLinked.RecordCount > 0)
                                        {
                                            objRecordSetNotLinked.MoveFirst();
                                            while (!objRecordSetNotLinked.EoF)
                                            {
                                                double dbJEVal = objRecordSetNotLinked.Fields.Item("LocTotal").Value;
                                                dbJEVal = Math.Round(dbJEVal, 2);
                                                if (dbJEVal == dbValue)
                                                {
                                                    strValue = Convert.ToString(objRecordSetNotLinked.Fields.Item("transid").Value);
                                                    intResult = 0;
                                                    try
                                                    {
                                                        SAPbobsCOM.JournalEntries oJournal = BYBB1MainObject.Instance.B1Company.GetBusinessObject(SAPbobsCOM.BoObjectTypes.oJournalEntries);
                                                        if(oJournal.GetByKey(objRecordSetNotLinked.Fields.Item("transid").Value))
                                                        {
                                                            oJournal.Memo = "Autoretención para el documento " + oDoc.DocNum + " de " + strCode;
                                                            oJournal.Reference3 = Convert.ToString(oDoc.DocEntry);
                                                            int ir = oJournal.Update();
                                                            if(ir!= 0)
                                                            {
                                                ...
                                                objRecordSetNotLinked.MoveNext();
                                            }
                                        }
                                        ...
                                        if (strValue.Length == 0)
                                        {
                                            ...
                                            if (strDocDate == "Y")
                                            {
                                                ...
                                                objJE.ReferenceDate = oDoc.TaxDate;
                                                objJE.DueDate = oDoc.TaxDate;
                                            }
                                            else
                                            {

                                                objJE.TaxDate = BYBHelpers.Instance.getDateTimeFormString(strDateValue);
                                                objJE.ReferenceDate = BYBHelpers.Instance.getDateTimeFormString(strDateValue);
                                                objJE.DueDate = BYBHelpers.Instance.getDateTimeFormString(strDateValue);
                                            }
                                            ...
                                            if (intResult == 0)
                                            {
                                                strValue = BYBB1MainObject.Instance.B1Company.GetNewObjectKey();
                                            }
                                        }
                                ...
                                {
                                    BYBB1MainObject.Instance.B1Application.MessageBox("El documento " + strDocNum + " no se contabilizó ya que la autoretención no se encuentra activa para el socio de negocio.");
                                }
                ...
                {
                    #region specialCancel

                    if (T1.Properties.Settings.Default.cancelstrCode)
                    {
                        string strSQL1 = "select distinct transid from JDT1 where linememo like '%strCode%' and transid not in (select U_JEEntry from[@BYB_T1SWT200])";
                        SAPbobsCOM.Recordset objRecordSetNotLinked1 = BYBB1MainObject.Instance.B1Company.GetBusinessObject(SAPbobsCOM.BoObjectTypes.BoRecordset);
                        objRecordSetNotLinked1.DoQuery(strSQL1);
                        if (objRecordSetNotLinked1.RecordCount > 0)
                        {
                            int intFound = objRecordSetNotLinked1.RecordCount;
                            objRecordSetNotLinked1.MoveFirst();
                            while (!objRecordSetNotLinked1.EoF)
                            {
                                int strJEVal = objRecordSetNotLinked1.Fields.Item("transid").Value;

                                SAPbobsCOM.JournalEntries oJournal1 = BYBB1MainObject.Instance.B1Company.GetBusinessObject(SAPbobsCOM.BoObjectTypes.oJournalEntries);
                                if (oJournal1.GetByKey(strJEVal))
                                {
                                    try
                                    {
                                        int ir = oJournal1.Cancel();
                                ...
                                objRecordSetNotLinked1.MoveNext();
                            }
